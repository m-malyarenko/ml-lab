...
data <- read.table("data/reglab1.txt", sep = "\t", header = TRUE)

predictors <- data.matrix(data[ , c('x', 'z')])
response <- data$y
...
rss_r_sqr <- r.sqr(data$y, rss$fitted.values)
rss_rmse <- rmse(data$y, rss$fitted.values)
