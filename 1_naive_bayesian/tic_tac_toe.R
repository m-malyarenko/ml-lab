...
data_raw <- read.table("data/tic_tac_toe.csv", sep = ",", stringsAsFactors = TRUE)

n <- nrow(data_raw)

set.seed(128)

data_shuffled <- data_raw[order(runif(n)), ]
...
for (train_ratio in c(0.5, 0.6, 0.7, 0.8, 0.9)) {
  ...
  bayes_classifier <- naiveBayes(data_train[,-10], data_train$V10)
